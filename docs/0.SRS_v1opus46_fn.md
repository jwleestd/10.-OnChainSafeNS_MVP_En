...
- **v0.8 보완: 바이브코딩 적합성 + 시스템·비용 효율성 통합 조정 (18건) — (1) 3계층→2계층 아키텍처 완화 (2) SQLite/PostgreSQL 이중환경→Supabase PostgreSQL 단일 (3) T7 결정에 따라 중앙 API 응답 헬퍼 필수화 (4) Rate Limit Phase-0 구현 생략 (5) T8 결정에 따라 Phase-0 Playwright E2E 필수화 (6) 시드 30건+faker 가이드 (7) Phase-1→1a/1b 분할 (8) Cron 60초 제한 대응 (9) MistTrack·ScamSniffer 가용성 사전검증 (10) OFAC SDN 유형 재분류 (11) Merkle 앵커링 Phase-1b→Phase-2 이동 (12) Chainalysis Phase-3 확정+Zero-FP 무상전략 명시 (13) L2 체인 후보+가스비 산정 (14) Resend 100통/일 한도 대응 (15) KakaoTalk Phase-1 제외 (16) 커스텀 도메인 비용 추가 (17) Phase별 비용 재산정 (18) 바이브코딩 설계 원칙 추가**
...
**v0.8 핵심 변경:** 바이브코딩 적합성 + 시스템·비용 효율성 통합 조정. (1) 3계층→2계층 아키텍처로 완화하여 AI 코드 생성과 구조 일치 (2) SQLite/PostgreSQL 이중환경을 Supabase PostgreSQL 단일로 통합하여 환경 분기 에러 원천 제거 (3) Phase-1을 1a(CRUD 확장)·1b(블록체인·외부연동)로 분할하여 난이도 절벽 해소 (4) Chainalysis를 Phase-3으로 확정하고 Phase-2 Zero-FP는 무상 소스 전략으로 전환 (5) Phase별 비용 재산정 (6) T8 결정에 따라 Phase-0부터 Playwright E2E를 필수 Gate로 적용
...
| **P6** | **바이브코딩 호환** | **(v0.8 신규, T7 보정)** AI 코드 생성과 구조가 일치하도록 설계. Route Handler 내에서 직접 Prisma 호출 허용(2계층). 비즈니스 서비스 모듈은 만들지 않되, API 응답 envelope와 에러 처리는 `lib/api-response.ts` 중앙 헬퍼로 필수 분리 |
...
**Phase-0 지원 체인 (v0.9 확정 — 4종):**

| 체인 ID | 표시명 | 주소 형식 | 검증 정규식 | 비고 |
|---|---|---|---|---|
| `ethereum` | Ethereum | `0x` + 40 hex | `/^0x[0-9a-fA-F]{40}$/` | EVM 기본 |
| `polygon` | Polygon | `0x` + 40 hex | `/^0x[0-9a-fA-F]{40}$/` | EVM 호환 |
| `bsc` | BNB Smart Chain | `0x` + 40 hex | `/^0x[0-9a-fA-F]{40}$/` | EVM 호환 |
| `solana` | Solana | Base58, 32~44자 | `/^[1-9A-HJ-NP-Za-km-z]{32,44}$/` | 별도 패턴 |

> `lib/validators.ts`의 `validateAddress(address, chain)` 함수에서 체인별 정규식을 적용한다. `lib/constants.ts`의 `SUPPORTED_CHAINS` 배열에 4종을 정의하고, UI의 체인 선택 `Select` 드롭다운에 동일 목록을 사용한다.
...
| ID | 제약 | Phase | 비고 |
|---|---|---|---|
| C-TEC-001 | Next.js App Router 단일 풀스택 (**v14.2.x 안정 버전 확정**) | Phase-0~ | — |
...
| C-TEC-004 | Tailwind CSS + shadcn/ui. **next-themes 적용하여 라이트/다크 시스템 테마 연동 지원** (§3.2.3 참조) | Phase-0~ | — |
...
| **C-P0-008** | **(v0.8 신규, T8 확정)** Playwright E2E: Phase-0부터 필수 Gate로 적용. REQ-P0-001~019에 대응하는 19개 핵심 시나리오 전체 green이 Phase-1a 전환 조건이며, 수동 체크리스트는 보조 검증으로만 사용 | 정책 |
...
| CON-15 | 비용: P0 $22, P1a $86, P1b $140, Prod $500 이하 | Phase-0~ | 가정 |
...
#### 3.2.1 Phase-0 페이지 (5개)

| # | 라우트 | 기능 | 복잡도 |
|---|---|---|---|
| 0 | `/` | **랜딩 페이지** — 제품 소개(한 줄 비전), 핵심 기능 3종 CTA(사기 조회 / Safe-Name / 데모 이체), 하단 간략 통계(시드 DB 건수 등) | 낮음 |
| 1 | `/(public)/fraud-lookup` | 사기 주소 조회 + 신고 | 낮음 |
| 2 | `/(public)/safe-name` | 이름 등록 → 리졸브 확인 → **데모 이체** 순차 Wizard | 중간 |
| 3 | `/(admin)/approval` | 신고 승인 대시보드 | 낮음 |
| 4 | `/auth/verify` | 이메일 인증 | 낮음 |

**T1 UX 진입 결정:** Phase-0 첫 화면(`/`)은 랜딩 페이지 + 핵심 기능 3종 CTA 구조로 확정한다. 구현 시 `/`에 사기 조회 입력폼을 직접 배치하거나 모든 공개 기능을 단일 콘솔로 합치지 않는다. 기능 실행은 `/fraud-lookup`, `/safe-name`, `/auth/verify` 등 명시 라우트에서 수행한다.

**T2 인증 UX 결정:** 랜딩 페이지(`/`)는 인증 없이 접근 가능하다. 랜딩 CTA 또는 네비게이션을 통해 `/fraud-lookup`, `/safe-name` 등 기능 소개·실행 페이지로 진행하려는 미인증 사용자는 `/auth/verify?redirect={대상경로}`로 먼저 이동한다. 이메일 인증 성공 후 `redirect` 대상 경로로 자동 복귀한다.

**T3 Safe-Name UX 결정:** `/safe-name`은 등록·리졸브·데모 이체를 독립 탭으로 나누지 않고 순차 Wizard로 제공한다. Step 1에서 Safe-Name을 등록하고, Step 2에서 등록 또는 입력된 이름의 리졸브·사기 상태를 확인하며, Step 3에서 확인된 이름을 수신자로 데모 이체를 실행한다. 각 단계는 이전 단계의 값을 프리필하되 사용자가 수정할 수 있다.

**T4 사기 조회-신고 전환 결정:** `/fraud-lookup`은 조회 결과 카드에 "이 주소 신고하기" 액션을 제공한다. 사용자가 해당 액션을 선택하면 같은 페이지의 신고 영역으로 이동하며, 조회에 사용한 주소와 체인이 신고 폼에 자동 프리필된다. 사용자는 피해 내역과 증빙 URL만 추가 입력하고 제출한다.
...
|---|---|---|
| 상단 Nav 좌측 | 로고/제품명 | `/` |
| 상단 Nav 중앙 | 홈 \| 사기 조회 \| Safe-Name | `/` \| 미인증 시 `/auth/verify?redirect=/fraud-lookup`, 인증 시 `/fraud-lookup` \| 미인증 시 `/auth/verify?redirect=/safe-name`, 인증 시 `/safe-name` |
| 상단 Nav 우측 | 인증 상태 표시 (미인증: "이메일 인증" 버튼 → `/auth/verify`, 인증 완료: 이메일 표시) | — |
| Admin 진입 | 상단 Nav에 노출하지 않음. `/auth/admin-login` 직접 접근 | — |
...
|---|---|---|
| `fraud-lookup` | `Input`, `Select`, `Button`, `Card`, `Badge`, `Alert` | 주소 입력, 체인 선택, 조회 결과 카드, 위험 등급 배지, 경고 알림. 결과 카드에 **"이 주소 신고하기"** 액션 제공 |
| `fraud-lookup` 신고 영역 | `Textarea`, `Input`, `Button`, `Dialog`, `Toast` | 조회 결과 카드의 신고 액션으로 진입. 피해 내역 입력, 증빙 URL, 신고 확인 다이얼로그, 접수 완료 토스트. **조회에 사용한 주소+체인이 자동 프리필됨 (수정 가능)** |
| `safe-name` Wizard Step 1 | `Input`, `Select`, `Button`, `Toast` | Safe-Name, 체인, 지갑 주소 입력 후 등록. 완료 시 Step 2로 이동하며 이름을 프리필 |
| `safe-name` Wizard Step 2 | `Input`, `Button`, `Card`, `Badge`, `Alert` | 이름 리졸브 결과와 사기 상태 확인. 확인된 이름·주소·체인을 Step 3으로 전달 |
| `safe-name` Wizard Step 3 | `Input`, `Button`, `Card`, `Alert`, `Toast`, `Table` | 수신자 이름·금액 입력, 데모 이체 실행, 완료/차단 결과 카드. **하단에 "내 이체 기록" 최근 5건 리스트 표시 (인증 유저만, TRANSFER_DEMO 테이블 조회)** |
| `approval` | `Table`, `Button`, `Badge`, `Dialog`, `Textarea`, `Toast` | 대기 신고 목록, 주소·체인·피해 내역·증빙 URL·신고 시각, 동일 주소 신고 수, 기존 FRAUD_ADDRESS 상태, 신고자 허위 신고 누적 수, 승인/거부 버튼, 상태 배지, 거부 사유 입력 |
| `auth/verify` | `Input`, `Button`, `InputOTP`, `Toast` | 이메일 입력, 6자리 코드 입력(OTP), 인증 완료 토스트 |
| `/` (랜딩) | `Button`, `Card`, `Badge` | CTA 버튼 3종(조회/등록/이체), 기능 소개 카드, 시드 DB 건수 배지 |
| 공통 (layout) | `NavigationMenu`, `Separator` | 상단 네비게이션(홈·사기조회·Safe-Name), 구분선 |
| `safe-name` Wizard 공통 | `Stepper` 또는 shadcn `Card`+`Badge` 조합 | 1 등록 → 2 리졸브 확인 → 3 데모 이체의 현재 단계 표시와 이전/다음 이동 |

**미인증 유저 UX 패턴 (T2 확정):**

> 랜딩 페이지는 인증 없이 열람 가능하다. 기능 소개·실행 페이지는 `user_session` 쿠키 미존재 시 아래 패턴을 적용한다.

| 상태 | UI 동작 |
|---|---|
| 미인증 (`user_session` 없음) | `/fraud-lookup`, `/safe-name` 등 기능 페이지 진입 시 `/auth/verify?redirect={현재경로}`로 리다이렉트. 랜딩 CTA도 동일 redirect URL을 사용 |
| 인증 완료 후 복귀 | `/auth/verify`에서 인증 성공 → `redirect` 쿼리 파라미터의 원래 페이지로 자동 복귀 |
| 인증 완료 상태 | 기능 소개·실행 페이지 정상 접근 및 폼 활성화 |
...
> **T7 예외:** API 응답 envelope와 에러 처리는 반복 횟수와 무관하게 Phase-0부터 `lib/api-response.ts`로 분리한다. 이는 서비스 계층 도입이 아니라 Route Handler 응답 계약을 통일하기 위한 공통 유틸리티다.
...
sequenceDiagram
    ...
    participant RH as Route Handler (route.ts)
    ...
    RH->>DB: fraud_address 쿼리 (직접 호출)
    DB-->>RH: 결과 (시드 데이터)
    RH-->>Page: 위험 등급, 건수, 소스

    User->>Page: 조회 결과 카드의 [이 주소 신고하기] 선택
    Page-->>User: 신고 영역으로 이동 + 주소/체인 프리필
    User->>Page: 피해 내역 + 증빙 URL 입력 후 제출
    Page->>RH: POST /fraud/report
    RH->>Auth: 쿠키(user_session) 검증
    Auth-->>RH: 검증 결과
    alt 미인증
        RH-->>Page: HTTP 401
    else 인증 완료
        RH->>DB: FRAUD_REPORT 생성
        RH->>Email: 접수 확인 Email
        RH-->>Page: "신고 접수 완료"
    end
```
...
sequenceDiagram
    ...
    RH->>Auth: 쿠키(user_session) 검증
    alt 미인증
        RH-->>Page: HTTP 401 "이메일 인증 필요"
        User->>Page: 이메일 → 코드 수신 → 입력
        Page->>RH: POST /auth/verify-email
        RH->>DB: USER.email_verified = true
        RH-->>Page: Set-Cookie: user_session 생성
    else 인증 완료
        RH->>DB: 중복 확인 + SAFE_NAME 생성 (이름.safe)
        RH->>Email: 등록 완료 Email
        RH-->>Page: "등록 완료!"
    end

    User->>Page: [리졸브] → "kim.safe"
    Page->>RH: GET /resolve?name=kim.safe
    RH->>DB: SAFE_NAME 조회 (.safe 부착 조회)
    RH->>DB: FRAUD_ADDRESS 교차
    alt 사기 매칭
        RH-->>Page: "⚠ 사기 이력"
    else 정상
        RH-->>Page: "✅ kim.safe → 0x..."
...
sequenceDiagram
    ...
    Admin->>Page: 대시보드 접속 (/admin/approval)
    Page->>MW: 페이지 요청
    MW->>MW: 쿠키(admin_session) 검증
    MW-->>Page: 통과
    Page->>RH: GET /admin/pending-reports (또는 RSC 직접 DB)
    RH->>DB: FRAUD_REPORT WHERE submitted
    DB-->>Page: 대기 목록

    Admin->>Page: [승인] / [거부] (사유: 허위 신고)
    Page->>RH: POST /admin/approve
    RH->>DB: status→verified / rejected
    alt 승인
        RH->>DB: FRAUD_ADDRESS 생성/갱신 (자동 상향 로직)
    else 허위 신고 거부
        RH->>DB: USER.false_report_count += 1 (Auto-ban 처리)
    end
    RH->>Email: 신고자에게 결과 통지
```
...
sequenceDiagram
    ...
    participant Page as safe-name Wizard Step 3
    participant RH as Route Handler
    participant Auth as Auth Utils
    participant DB as Prisma DB
    participant Email as Resend

    A->>Page: Wizard Step 3 → 수신자 이름 "kim.safe" + 금액 "0.5" 입력
    Page->>RH: POST /api/v1/transfer/demo {recipient_name: "kim.safe", amount: 0.5}
    RH->>Auth: 쿠키(user_session) 검증
    
    RH->>DB: SAFE_NAME 리졸브 ("kim.safe")
    alt 이름 미등록
        DB-->>RH: 미등록
        RH-->>Page: HTTP 404 "해당 이름은 등록되지 않았습니다"
    else 이름 등록됨
        DB-->>RH: wallet_address=0xd8dA..., chain=ethereum
        RH->>DB: FRAUD_ADDRESS 교차 조회 (0xd8dA...)
        alt ⚠ 사기 DB 매칭
            DB-->>RH: risk_level=critical, report_count=3
            RH->>DB: TRANSFER_DEMO 생성 (status='blocked')
            RH->>Email: 차단 Email (이름, 주소, 금액, 사유)
            RH-->>Page: "🚫 이체 차단 — kim.safe (0xd8dA...) 사기 이력 3건"
        else ✅ 정상
            RH->>DB: TRANSFER_DEMO 생성 (status='completed')
            RH->>Email: 완료 Email (이름, 주소, 체인, 시뮬레이션)
            RH-->>Page: "✅ 이체 완료 (시뮬레이션) — kim.safe / 0.5 ETH"
        end
    end
```
...
| 쿠키명 | 대상 | 값 | httpOnly | Secure | SameSite | 만료 |
|---|---|---|---|---|---|---|
| `admin_session` | 운영자 | `admin_id:hmac_signature` (HMAC 서명) | ✅ | ✅ (Prod) | `Lax` | **기본 30분** (`ADMIN_SESSION_MINUTES` 환경변수로 설정 가능) |
| `user_session` | 일반 유저 | `user_id:hmac_signature` (HMAC 서명) | ✅ | ✅ (Prod) | `Lax` | **7일** |

> **user_session 동작:** POST `/api/v1/auth/verify-email` 성공 시 `Set-Cookie: user_session={user_id}:{signature}; HttpOnly; Path=/; Max-Age=604800` 설정. 이후 인증 필요 API에서 쿠키의 `user_id`로 DB 조회하여 `email_verified=true` 확인.
> **admin_session 동작:** POST `/auth/admin-login` 성공 시 `Set-Cookie: admin_session={admin_id}:{signature}; HttpOnly; Path=/; Max-Age={ADMIN_SESSION_MINUTES*60}` 설정.
> **서명 방식:** `crypto.createHmac('sha256', ADMIN_PASSWORD).update(id).digest('hex')`. 별도 SESSION_SECRET 환경변수 추가 없이 기존 변수 재활용.

**Admin 로그인 페이지 (`/auth/admin-login`) 명세 (v0.9 확정):**

| 항목 | 상세 |
|---|---|
| 폼 구성 | **Admin ID** 입력 + **비밀번호** 입력 (향후 다중 Admin 대비) |
| 인증 검증 | `admin_id === ADMIN_ID` && `password === ADMIN_PASSWORD` 일치 시 쿠키 발급 |
| 성공 시 | `/(admin)/approval`로 리다이렉트 |
| 실패 시 | "인증 정보가 일치하지 않습니다" Toast 표시 |
| shadcn/ui | `Input`(ID), `Input`(PW, type="password"), `Button`, `Toast` |

**Admin 로그아웃 (v0.9 확정):**

| 항목 | 상세 |
|---|---|
| 위치 | `/(admin)/approval` 페이지 상단 우측 "로그아웃" 버튼 |
| 동작 | `admin_session` 쿠키 삭제 (`Max-Age=0`) → `/auth/admin-login` 리다이렉트 |
| 구현 | `/api/v1/admin/logout` POST Route Handler 또는 클라이언트 사이드 쿠키 삭제 |

**보호 대상 경로:**

| Path Pattern | 인증 방식 | 미인증 시 동작 |
|---|---|---|
| `/fraud-lookup`, `/safe-name` | 일반 유저 쿠키 (`user_session`) 확인 | `/auth/verify?redirect={현재경로}` 리다이렉트 |
...
이메일 인증이 필요한 API(`/api/v1/fraud/lookup`, `/api/v1/fraud/report`, `/api/v1/resolve`, `/api/v1/safename/register`, `/api/v1/transfer/demo`)는 middleware가 아닌 각 Route Handler 내부에서 쿠키 `user_session`을 파싱하여 `user_id`를 추출하고, DB에서 `email_verified=true`를 확인한다.

```
유저 인증 검증 의사 코드:
1. req.cookies.get('user_session') → signed_value
2. signed_value에서 user_id + signature 분리
3. HMAC(ADMIN_PASSWORD, user_id) === signature 검증 → 실패 시 401
4. prisma.user.findUnique({ where: { user_id } }) → email_verified 확인 → false 시 401
5. 검증 통과 → user_id를 비즈니스 로직에 전달
```
...
**T7 통합 API 응답 결정:** Phase-0의 모든 `/api/v1/*` Route Handler는 `lib/api-response.ts`의 중앙 응답 헬퍼를 사용한다. `successResponse(data, status?)`, `errorResponse(code, message, status, details?)`, `withErrorHandler(handler)`를 API-001의 필수 산출물로 구현하고, 각 route는 동일한 `success`, `data`/`error`, `meta.timestamp`, `meta.request_id` envelope를 반환해야 한다. 엔드포인트별 임의 에러 구조와 route-local `NextResponse.json()` 반복 작성은 허용하지 않는다.

**T8 테스트 Gate 결정:** Phase-0 구현 완료와 Phase-1a 전환은 Playwright E2E 전체 green을 필수 조건으로 삼는다. §6.9의 19개 REQ-P0-001~019 시나리오는 Playwright spec으로 자동화하고, 수동 Pass/Fail 체크리스트는 데모 리허설과 브라우저 육안 확인용 보조 자료로 유지한다.

**T9 시드 규모 결정:** Phase-0 기본 seed 규모는 `FRAUD_ADDRESS 30건 + SAFE_NAME 10건 + USER 5건 + OPERATOR 1건`으로 고정한다. 확장성 수용 검증은 `FRAUD_REPORT 100건`을 추가로 생성해 확인하며, Phase-0에서 `FRAUD_ADDRESS 100건 + SAFE_NAME 20건 + FRAUD_REPORT 500건` 기준을 요구하지 않는다.

**T10 비용 Gate 결정:** 비용 기준은 커스텀 도메인 월 환산 비용을 포함한 SRS 기준으로 통일한다. Phase-0 완료 Gate는 월 `≤ $22`, Phase-1a Gate는 월 `≤ $86`, Phase-1b→Phase-2 Gate는 월 `≤ $140`을 사용한다. 기존 `$21`, `~$71`, `≤ $100` 문구는 Phase Gate 기준으로 사용하지 않는다.
...
| REQ-P0-004 | 동일 주소 중복 신고 시 안내 | Must | **[정상]** Given: 동일 reported_address+chain의 기존 FRAUD_REPORT 또는 FRAUD_ADDRESS가 존재 / When: 다른 유저가 동일 주소 신고 / Then: 신규 FRAUD_REPORT만 생성 (FRAUD_ADDRESS는 관리자 승인 시에만 생성/갱신됨). 신고자에게 "이미 N건 신고/등록된 주소입니다. 귀하의 신고가 추가 접수되었습니다" 메시지. **[실패]** Given: 동일 유저가 동일 주소 재신고 / When: 제출 / Then: HTTP 409 "이미 신고하신 주소입니다" |
...
**`.safe` 접미사 처리 규칙 (v0.9 확정):**

| 단계 | 동작 | 예시 |
|---|---|---|
| UI 입력 | 유저는 **접미사 없이** 이름만 입력. 입력 필드 우측에 `.safe` 레이블 고정 표시 | 입력: `alice` → 표시: `alice.safe` |
| API 수신 | `human_name` 파라미터에 `.safe` 없으면 서버가 자동 부착 | `"alice"` → `"alice.safe"` |
| DB 저장 | `human_name` 컬럼에 **`.safe` 포함하여 저장** | `alice.safe` |
| 이름 규칙 검증 | `.safe` 제거 후 본체만 검증 (영소문자·숫자·하이픈, 3~20자) | `alice` 검증 → 통과 |
| 리졸브 입력 | `alice` 또는 `alice.safe` **둘 다 허용**. `.safe` 없으면 자동 부착 후 조회 | `GET /resolve?name=alice` = `GET /resolve?name=alice.safe` |

**예약어(Reserved Words) 차단 목록 (v0.9 확정):**
피싱 방지 및 시스템 사칭 방지를 위해 다음 단어들은 `human_name` 등록 시 차단(HTTP 400)한다. (추후 Phase-1에서 DB 기반 동적 관리로 전환 가능)
- **시스템/관리자:** `admin`, `root`, `system`, `master`, `support`, `help`, `info`, `official`, `team`, `manager`
- **기관/거래소/포털:** `police`, `gov`, `fss`, `upbit`, `bithumb`, `binance`, `kakao`, `naver`
- **프로토콜/체인:** `eth`, `btc`, `sol`, `polygon`, `safe`
...
| ID | 요구사항 | Priority | Acceptance Criteria |
|---|---|---|---|
| REQ-P0-011 | Admin 인증 운영자가 대기 신고 목록 조회 | Must | **[정상]** Given: Admin PW 인증 완료 운영자 / When: 승인 대시보드 접속 / Then: 3초 이내 FRAUD_REPORT WHERE status='submitted' 목록 로드. 각 항목에 report_id, reported_address, chain, description, evidence_url, reported_at, 동일 주소 신고 수, 기존 FRAUD_ADDRESS 상태, 신고자 false_report_count를 표시. **[실패]** Given: Admin PW 미인증 / When: 대시보드 접속 시도 / Then: HTTP 401 → 로그인 페이지 리다이렉트 |
| REQ-P0-012 | 건별 승인/거부. 승인 시 FRAUD_ADDRESS 생성/갱신 | Must | **[T5 품질 보강 기준]** 운영자는 동일 주소 신고 수, 기존 FRAUD_ADDRESS 상태, 신고자 false_report_count를 확인한 뒤 승인/거부한다. Phase-0에서는 위험도 수동 선택을 강제하지 않는다. **[승인]** Given: status='submitted' 신고 / When: 운영자가 [승인] 클릭 / Then: FRAUD_REPORT.status→'verified', FRAUD_REPORT.reviewed_at=now(). **해당 주소+체인의 FRAUD_ADDRESS가 없으면 이때 최초 생성(risk_level='medium', source_type='community'), 있으면 기존 FRAUD_ADDRESS.report_count += 1. (자동 상향 로직: 누적 report_count 5건 도달 시 risk_level='high', 10건 도달 시 risk_level='critical'로 갱신)**. **[거부]** Given: status='submitted' 신고 / When: 운영자가 코멘트 입력 후 [거부] 클릭 / Then: FRAUD_REPORT.status→'rejected', reviewer_notes 저장. **(허위 신고 자동 차단) 거부 사유가 "허위 신고"인 경우, 신고자의 `USER.false_report_count += 1`. 누적 3회 도달 시 `report_restriction_until = now() + 30일` 설정** |
| REQ-P0-013 | 결과 Email 통지 | Must | **[정상]** Given: 승인/거부 처리 완료 / When: 상태 전환 직후 / Then: 30초 이내 신고자에게 Email 발송. 승인 시 "귀하의 신고가 승인되어 사기 DB에 반영되었습니다", 거부 시 "귀하의 신고가 거부되었습니다. 사유: {reviewer_notes}". 성공률 ≥ 95%. **[실패]** Given: Resend API 장애 / When: 메일 발송 실패 / Then: 에러 로그 기록(Vercel Logs). (Phase-0 공통 정책: 발송 실패 시 자동 재시도 없음) |

#### P0-F6. 이메일 인증

| ID | 요구사항 | Priority | Acceptance Criteria |
|---|---|---|---|
| REQ-P0-014 | 이메일 → 6자리 코드 발송. 유효 10분 | Must | **[정상]** Given: 유저가 이메일 주소 입력 / When: [인증 요청] 클릭 / Then: 5초 이내 6자리 숫자 코드 Email 발송. verification_code DB 저장, verification_expires_at = now() + 10분. **[실패-1]** Given: 이메일 형식 불일치 / When: 인증 요청 / Then: HTTP 400 "올바른 이메일을 입력하세요". **[실패-2]** Given: 동일 이메일로 1분 이내 재요청 / When: 인증 요청 / Then: HTTP 429 "잠시 후 다시 시도하세요". **[실패-3]** Given: Resend API 장애 / When: 인증 코드 발송 실패 / Then: HTTP 502 "인증 이메일 발송에 실패했습니다. 잠시 후 다시 시도하세요", USER 인증 상태는 변경하지 않음 |
...
**인증 성공 응답 규칙 (v0.9 확정 — 데이터 최소화 원칙):**

| 항목 | 상세 |
|---|---|
| 응답 본문 | `{ "success": true }` 만 반환. `user_id`, `email` 등 유저 정보 미포함 |
| 인증 토큰 | `Set-Cookie: user_session` httpOnly 쿠키로만 전달 (§3.7 참조) |
| 클라이언트 이메일 표시 | 유저가 입력한 이메일을 클라이언트 React state에 보관하여 Nav 우측에 표시. 새로고침 시 `user_session` 쿠키 존재 여부로 인증 상태만 판단, 이메일은 미표시 |

**이메일 발송 실패 정책 (T6 확정):**

| 이메일 유형 | 실패 시 처리 |
|---|---|
| 인증 코드 발송 | 유저 진행이 불가능하므로 HTTP 502로 실패 응답, 에러 로그 기록, USER 인증 상태 변경 없음 |
| 신고 접수 확인 | FRAUD_REPORT 생성은 정상 완료, 에러 로그 기록, 자동 재시도 없음 |
| 승인/거부 결과 통지 | FRAUD_REPORT/FRAUD_ADDRESS 상태 변경은 정상 완료, 에러 로그 기록, 자동 재시도 없음 |
| 데모 이체 결과 통지 | TRANSFER_DEMO 생성은 정상 완료, 에러 로그 기록, `notified_at=null` 유지, 자동 재시도 없음 |
...
| REQ-P0-NF-005 | 신뢰성 | 시드 정합성 | FRAUD_ADDRESS 30건 + SAFE_NAME 10건 + USER 5건 + OPERATOR 1건 정확도 100% | prisma db seed 후 COUNT 자동 검증 | 불일치 시 seed.ts 수정 → 재실행 |
...
| REQ-P0-NF-008 | 비용 | 월 인프라 | ≤ $22 (Vercel $0~20 + Supabase $0 + Resend $0 + 도메인 약 $1) | 대시보드 월 1회 확인 | $22 초과 시 최적화 또는 Phase-1 조기 전환 |
...
| REQ-NF-024 | 인프라 비용 | P0≤$22, P1a≤$86, P1b≤$140, Prod≤$500 | 전체 SaaS 월 합산 | 초과 시 비용 분석 → 최적화 또는 Phase 전환 | 0~ |
...
|---|---|---|---|---|
| P0-A1 | `/api/v1/fraud/lookup` | GET | Email인증 | 유저 100/min |
| P0-A2 | `/api/v1/fraud/report` | POST | Email인증 | 유저 10/day |
| P0-A3 | `/api/v1/resolve` | GET | Email인증 | 유저 200/min |
...
#### 6.1.1 통합 에러 응답 포맷 (T7 확정: 중앙 헬퍼 필수)

> **T7 확정:** Phase-0에서는 통합 응답 포맷을 권장 수준으로 두지 않고 필수 구현 기준으로 적용한다. 모든 `/api/v1/*` Route Handler는 `lib/api-response.ts`의 `successResponse`, `errorResponse`, `withErrorHandler`를 사용해 동일한 성공/에러 envelope와 에러 코드를 반환한다.
...
| status | String | DEFAULT 'verified' | 상태 (`verified`, `inactive`, `appealed`) |
...
| evidence_url | String | NOT NULL | 증빙 URL |
...
> T9 확정: 시드 데이터를 100+20+5+1건에서 **30+10+5+1건으로 축소**한다. 확장성 수용 검증은 FRAUD_REPORT 100건 기준으로 수행한다. `@faker-js/faker` 라이브러리를 활용하여 seed.ts 작성 부담을 최소화한다.
...
│   │   └── safe-name/
│   │       └── page.tsx               ← 등록 → 리졸브 확인 → 데모 이체 순차 Wizard
│   ├── (admin)/
│   │   └── approval/
│   │       └── page.tsx               ← 신고 승인 대시보드
│   ├── auth/
...
│   ├── layout.tsx
│   ├── page.tsx                       ← 랜딩 페이지 (제품 소개 + CTA 3종)
│   └── globals.css
├── components/
│   ├── ui/                            ← shadcn/ui 자동 생성 (§3.2.3 참조)
│   └── shared/                        ← 프로젝트 공유 컴포넌트
├── lib/
│   ├── api-response.ts                ← 통합 API 응답 envelope + 에러 헬퍼 (T7 필수)
...
> **v0.7과의 차이:** `lib/services/fraud.ts`, `lib/services/safe-name.ts`, `lib/services/auth.ts`는 제거되었다. 비즈니스 로직은 각 `route.ts` 내에 직접 구현하되, T7 결정에 따라 API 응답 envelope와 에러 처리는 `lib/api-response.ts`에서 중앙 공유한다. 이메일·검증·DB 유틸리티도 `lib/`에서 공유한다.
...
| `ADMIN_ID` | Admin 로그인 ID | `admin` | Vercel: 별도 설정 |
| `ADMIN_PASSWORD` | Admin 인증 비밀번호 + HMAC 서명 키 | `dev-admin-pw-2026` | Vercel: 강력한 비밀번호 (≥16자) |
| `ADMIN_SESSION_MINUTES` | Admin 세션 만료 시간(분) | `30` | Vercel: `30` (운영자 필요 시 조정 가능) |
...
### 6.9 Phase-0 Playwright E2E 체크리스트 (T8 확정)

> Phase-0에서 필수로 자동화하는 Playwright E2E 검증 목록. 각 시나리오는 Playwright spec으로 작성하고 CI 또는 로컬 검증에서 전체 green이어야 한다. 필요 시 동일 목록을 수동 데모 리허설 체크리스트로도 사용한다.
...
> T8 확정: Phase-0부터 Playwright E2E를 필수 Gate로 적용한다. 수동 체크리스트는 보조 검증으로만 사용한다.

| # | 조건 | 검증 | Pass 기준 |
|---|---|---|---|
| 1 | REQ-P0-001~019 **Playwright E2E** 100% | §6.9 Playwright spec | 19개 시나리오 전체 green (실패 0건) |
...
| # | 조건 | 검증 | Pass 기준 |
|---|---|---|---|
| 1 | Phase-1a 추가 **Playwright E2E** 통과 | Playwright | Phase-0 19건 유지 green + Phase-1a 추가 테스트 전체 green |
