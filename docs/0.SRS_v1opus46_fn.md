...
|---|---|---|
| 상단 Nav 좌측 | 로고/제품명 | `/` |
| 상단 Nav 중앙 | 홈 \| 사기 조회 \| Safe-Name | `/` \| `/fraud-lookup` \| `/safe-name` |
...
| `fraud-lookup` (신고 탭) | `Textarea`, `Input`, `Button`, `Dialog`, `Toast` | 피해 내역 입력, 증빙 URL, 신고 확인 다이얼로그, 접수 완료 토스트. **조회 탭에서 입력한 주소+체인이 자동 프리필됨 (수정 가능)** |
...
| `safe-name` (이체 탭) | `Input`, `Button`, `Card`, `Alert`, `Toast`, `Table` | 수신자 이름·금액 입력, 이체 결과(완료/차단) 카드. **하단에 "내 이체 기록" 최근 5건 리스트 표시 (인증 유저만, TRANSFER_DEMO 테이블 조회)** |
...
| 공통 (layout) | `Tabs`, `NavigationMenu`, `Separator` | 페이지 내 탭 전환, 상단 네비게이션(홈·사기조회·Safe-Name), 구분선 |

**미인증 유저 UX 패턴 (v0.9 확정):**

> 이메일 인증이 필요한 기능(신고, 등록, 이체) 영역에 `user_session` 쿠키 미존재 시 아래 패턴을 적용한다.

| 상태 | UI 동작 |
|---|---|
| 미인증 (`user_session` 없음) | 해당 기능 폼 영역 상단에 `Alert` variant="warning" 표시: **"이메일 인증이 필요합니다"** + `Button` "이메일 인증하기" → `/auth/verify?redirect={현재경로}` 리다이렉트. 폼 입력 필드는 `disabled` 처리 |
| 인증 완료 후 복귀 | `/auth/verify`에서 인증 성공 → `redirect` 쿼리 파라미터의 원래 페이지로 자동 복귀 |
| 인증 완료 상태 | Alert 미표시, 폼 정상 활성화 |
...
이메일 인증이 필요한 API(`/api/v1/fraud/report`, `/api/v1/safename/register`, `/api/v1/transfer/demo`)는 middleware가 아닌 각 Route Handler 내부에서 쿠키 `user_session`을 파싱하여 `user_id`를 추출하고, DB에서 `email_verified=true`를 확인한다.
