...
| EXT-2 | 기관의 외부 사기정보 수집 역량 부재 | 자체 DB 커버리지 ≤ 30% | **P0: 시드 30건 + 신고 기반 신규 ≥ 20건** / P1: 무상 소스 3종 연동 | FRAUD_ADDRESS COUNT (source_type별) | **✅ Phase-0** |
...
| Phase-1 | Extended MVP | 4~6주 | 핫라인, 이의 신청, DNS 비용 모델, 멀티채널 알림, Fraud Agent | ~$86~$140 |
...
| P3 | **단일 인증** | Admin 비밀번호(`admin_session` 쿠키) + 이메일 인증(`user_session` httpOnly 쿠키, HMAC 서명, 7일 만료)만. JWT/RBAC 없음. 랜딩 페이지는 인증 없이 접근 가능하나, Phase-0 기능 소개·실행 페이지는 이메일 인증 후 진행한다 |
...
**Phase-0 UX 진입 원칙 (T1 확정):** 첫 화면(`/`)은 제품 소개 랜딩 페이지로 유지하고, 핵심 기능 3종 CTA(사기 조회, Safe-Name, 데모 이체)를 통해 기능 페이지로 진입한다. 실제 작업 화면은 `/(public)/fraud-lookup`, `/(public)/safe-name`, `/(admin)/approval`, `/auth/verify`로 분리하여 랜딩의 설명 역할과 도구 화면의 실행 역할을 구분한다.

**Phase-0 인증 UX 원칙 (T2 확정):** 랜딩 페이지(`/`)는 인증 없이 열람 가능하다. 랜딩의 CTA를 통해 사기 조회, Safe-Name, 데모 이체 등 기능 소개·실행 페이지로 진행하려는 사용자는 먼저 이메일 인증을 완료해야 하며, 인증 성공 후 원래 CTA 대상 경로로 복귀한다.

**Safe-Name Wizard 원칙 (T3 확정):** `/safe-name`은 탭 전환형 묶음이 아니라 순차 Wizard로 구성한다. 기본 흐름은 1단계 Safe-Name 등록, 2단계 리졸브 확인, 3단계 데모 이체 시뮬레이션이며, 사용자는 이전 단계 결과를 다음 단계 입력으로 이어받아 핵심 가치를 한 번의 안내 흐름으로 경험한다.

**사기 조회-신고 전환 원칙 (T4 확정):** `/fraud-lookup`은 조회 결과를 먼저 보여주고, 결과 카드에서 "이 주소 신고하기" 액션으로 신고 흐름에 진입한다. 신고 폼에는 조회에 사용한 주소와 체인을 자동 프리필하며, 사용자는 피해 내역과 증빙 URL만 추가 입력한다.

**이메일 발송 실패 정책 (T6 확정):** Phase-0에서는 중요도별 차등 처리한다. 이메일 인증 코드 발송 실패는 사용자가 다음 단계로 진행할 수 없으므로 요청 실패로 응답한다. 신고 접수 확인, 승인/거부 결과, 데모 이체 결과 통지 이메일은 발송 실패 시 Vercel Logs에 기록하고 핵심 DB 처리는 성공으로 유지하며 자동 재시도는 하지 않는다.

**통합 API 응답 정책 (T7 확정):** Phase-0의 모든 `/api/v1/*` Route Handler는 `lib/api-response.ts`의 중앙 헬퍼(`successResponse`, `errorResponse`, `withErrorHandler`)를 사용해 성공/에러 응답 envelope와 에러 코드를 통일한다. 각 `route.ts`에서 `NextResponse.json()`을 직접 반복 작성하거나 엔드포인트별 에러 구조를 허용하지 않는다.

**Phase-0 테스트 Gate 정책 (T8 확정):** Phase-0부터 Playwright E2E를 필수 완료 조건으로 적용한다. REQ-P0-001~019에 대응하는 19개 핵심 시나리오는 Playwright에서 전체 green이어야 하며, 수동 테스트 체크리스트는 보조 검증 및 데모 리허설 용도로만 사용한다.

**Phase-0 시드 규모 정책 (T9 확정):** Phase-0 기본 seed 규모는 SRS/Tasks 기준으로 통일한다. `FRAUD_ADDRESS 30건 + SAFE_NAME 10건 + USER 5건 + OPERATOR 1건`을 생성하고, 확장성 수용 검증은 `FRAUD_REPORT 100건` 기준으로 수행한다. `FRAUD_ADDRESS 100건 + SAFE_NAME 20건 + FRAUD_REPORT 500건` 기준은 Phase-0 완료 조건으로 사용하지 않는다.

**비용 Gate 정책 (T10 확정):** 비용 기준은 SRS의 커스텀 도메인 포함 산정으로 통일한다. Phase-0 완료 Gate는 월 `≤ $22`, Phase-1a Gate는 월 `≤ $86`, Phase-1b→Phase-2 Gate는 월 `≤ $140`을 사용한다. PRD의 기존 `$21`, `~$71`, `≤ $100` 기준은 더 이상 Phase Gate 기준으로 사용하지 않는다.
...
| **월 비용 (도입 기관)** | 무상 (제한적) | 무상 | $5,000~$50,000+/월 | **$1~$22** | $500~$1,000 |
| **B2C 직접 접근** | ❌ (API만) | ✅ (Chrome 한정) | ❌ (B2B만) | **✅ 웹 직접 접근** | ✅ |
| **오탐지율 (FP)** | 비공개 (BM-1에서 실측 예정) | 비공개 (BM-1에서 실측 예정) | ≤ 0.1% (공개 자료) | 시드 기반 0% (한정) | **≤ 0.01% 목표** |

**핵심 차별 포인트:** (1) 신고→승인→DB 반영 폐루프, (2) Safe-Name 이름 기반 이체, (3) 월 $22 이하 MVP 운영, (4) B2C 직접 접근 가능. 이 4가지는 기존 솔루션에서 동시에 제공하지 않는 조합이다.
...
| REQ-P0-012 | 건별 승인/거부. 승인 시 FRAUD_ADDRESS 자동 반영 | Must | **[T5 품질 보강 기준]** 운영자 승인 대시보드는 각 신고에 대해 report_id, reported_address, chain, description, evidence_url, reported_at과 함께 동일 주소 신고 수, 기존 FRAUD_ADDRESS 상태, 신고자 허위 신고 누적 수를 표시한다. **[승인]** Given: status='submitted' 신고 / When: 운영자가 [승인] 클릭 / Then: FRAUD_REPORT.status→'verified', FRAUD_REPORT.reviewed_at=now(). 해당 주소+체인의 FRAUD_ADDRESS가 없으면 신규 생성(risk_level='medium', source_type='community'), 있으면 report_count += 1. **[거부]** Given: status='submitted' 신고 / When: 운영자가 코멘트 입력 후 [거부] 클릭 / Then: FRAUD_REPORT.status→'rejected', reviewer_notes 저장 |
| REQ-P0-013 | 결과 Email 통지 | Must | **[정상]** Given: 승인/거부 처리 완료 / When: 상태 전환 직후 / Then: 30초 이내 신고자에게 Email 발송. 승인 시 "귀하의 신고가 승인되어 사기 DB에 반영되었습니다", 거부 시 "귀하의 신고가 거부되었습니다. 사유: {reviewer_notes}". 성공률 ≥ 95%. **[실패]** Given: Resend API 장애 / When: 메일 발송 실패 / Then: 에러 로그 기록, 승인/거부 DB 처리는 정상 완료, 자동 재시도 없음 |
...
| 4 | 월 비용 $140 이내 | 과금 | 전체 SaaS 30일 합산 ≤ $140 |
